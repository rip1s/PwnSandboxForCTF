...
a = asm(asmcode)
log.success('Original OEP:'+hex(e.entrypoint))
if e.is_pie:
...
if is64:
    oep = p64(segment.virtual_address)
else:
    oep = p32(segment.virtual_address)
log.success('NEW OEP:'+hex(segment.virtual_address))
e.write('test')
# Just overwrite the original oep
with open('test', 'rb+') as f:
    data = f.read()
    f.seek(0)
    if is64:
        f.write(data[:0x18] + oep + data[0x20:])
    else:
        f.write(data[:0x18] + oep + data[0x1c:])
    f.truncate()
    f.close()
